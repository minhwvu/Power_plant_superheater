...
def input_profile(t, x0):
    ...
    if t<60:
       ...
       x = x0*(1-(t-60)/1020*0.5)
    elif t<4680:
        x = x0*0.5
    elif t<5700:
        x = x0*(0.5+(t-4680)/1020*0.5)
    else: # hold for 1200 sec to 6900 sec
       x = x0
    return x
...
def main_dyn():
    ...
    itype_list = []
    for i in range(38): # to 2280
        itype_list.append(0)
    for i in range(20): # to 4680
        itype_list.append(1)
    for i in range(37): # to 6900
        itype_list.append(0)
...
def write_data_to_txt_file(plot_data):
    ...
    icount = 0
    with open("case2_3pct_result.txt","w") as fout:
        # write headings
        for k in plot_data:
            icount += 1
            fout.write(k)
