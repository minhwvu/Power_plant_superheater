...
def main():
    ...
    # m.fs.aPSH.tube_ncol.fix(88) # base
    # m.fs.aPSH.tube_ncol.fix(84)
    m.fs.aPSH.tube_ncol.unfix()
    m.fs.aPSH.tube_ncol.setlb(80)
    m.fs.aPSH.tube_ncol.setub(96)

    # m.fs.aPSH.tube_inlet_nrow.fix(4) # base
    ...
    # # step 2
    # # deactivate objective function 1
    # # activate objective function 2
    ...
    # multi_obj = pyo.sqrt(m.fs.omega_1*(obj1_f - obj1_opt)**2 + m.fs.omega_2*(obj2_f - obj2_opt)**2)
    # m.fs.multi_obj = pyo.Objective(expr=multi_obj, sense=pyo.minimize)

    # # run dynamic optimization for multiobjective
    # print('*****************************************************************************')
    # print('run multiobjective optimization')
    ...
    for t in m.fs.time:
        ...
        df_val.to_csv(f"opt_pressure_drop_stress_cons_case1.csv")
...
def write_data_to_txt_file(plot_data):
    ...
    icount = 0
    with open("result_pressure_drop_stress_cons_case1.txt","w") as fout:
        # write headings
        for k in plot_data:
            icount += 1
            fout.write(k)
