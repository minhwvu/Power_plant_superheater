...
def main():
    ...
    # def deltaP_base(b, t):
    #     return -(
    #         sum(b.aPSH.tube.deltaP[t, x] for x in b.aPSH.tube.length_domain
    #             ) - b.aPSH.tube.deltaP[t, b.aPSH.tube.length_domain.last()])*(b.aPSH.tube_nseg * b.aPSH.tube_length_seg
    #     )/b.aPSH.config.finite_elements
    ...
    m.fs.aPSH.tube_di.setlb(1.45*0.0254*0.9)
    m.fs.aPSH.tube_di.setub(1.45*0.0254*1.1)

    # m.fs.aPSH.tube_length_seg.value = 302.5*0.0254
    m.fs.aPSH.tube_length_seg.unfix()
    m.fs.aPSH.tube_length_seg.setlb(302.5*0.0254*0.9) # reduce from 0.8 to 0.5
    m.fs.aPSH.tube_length_seg.setub(302.5*0.0254*1.1)

    # case 5: continuous + discrete variables
    # m.fs.aPSH.tube_ncol.fix(88)
    ...
    # m.fs.outlet_enthalpy_cons.deactivate()
    # # objective function 1
    # m.fs.metal_mass = pyo.Objective(expr= const.pi*m.fs.aPSH.density_wall*m.fs.aPSH.tube_thickness*(
    #     m.fs.aPSH.tube_thickness+m.fs.aPSH.tube_di
    # )*m.fs.aPSH.tube_ncol*m.fs.aPSH.tube_inlet_nrow*m.fs.aPSH.tube_length_seg/metal_mass_base, sense=pyo.minimize)

    # # objective function 2
    # m.fs.obj2 = pyo.Objective(expr=sum(m.fs.aPSH.tube.properties[t, 0].pressure - m.fs.aPSH.tube.properties[t, 1].pressure for t in m.fs.time
    #     ),sense=pyo.minimize)

    # @m.fs.Objective(m.fs.time, sense = pyo.minimize)
    # def deltaP_objective(b, t):
    #     return -(sum(b.aPSH.tube.deltaP[t, x] for x in b.aPSH.tube.length_domain)
    #     - b.aPSH.tube.deltaP[t, b.aPSH.tube.length_domain.last()]) \
    #     * (b.aPSH.tube_nseg * b.aPSH.tube_length_seg)/b.aPSH.config.finite_elements/(max_pres_drop*nstep)

    m.fs.obj2 = pyo.Objective(expr=sum(
        m.fs.aPSH.tube_inlet.pressure[t] - m.fs.aPSH.tube_outlet.pressure[t] for t in m.fs.time
        )/(max_pres_drop*nstep), sense=pyo.minimize)

    # undo patch ipopt 
    # undo_patch_ipopt()

    # single objective optimization 2 (pressure drop)
    # m.fs.metal_mass.deactivate()
    # m.fs.deltaP_opt.activate()
    ...
    for t in m.fs.time:
        ...
        df_val.loc[t]["pressure_drop_opt"] = pyo.value(m.fs.deltaP_opt[t])
        ...
        df_val.to_csv(f"opt_pressure_drop_case5.csv")
    ...
    for t in m.fs.time:
        ...
        # plot_data['heat_duty'].append(pyo.value(m.fs.aPSH.total_heat[t]))
...
def write_data_to_txt_file(plot_data):
    ...
    icount = 0
    with open("SH_opt_pressure_drop_case5.txt","w") as fout:
        # write headings
        for k in plot_data:
            icount += 1
            fout.write(k)
