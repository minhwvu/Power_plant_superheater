...
def main():
    ...
    # print(nstep)
    if m.dynamic:
        m.fs = FlowsheetBlock(default={"dynamic": True, "time_set": time_10pct, "time_units": pyo.units.s})
    else:
    ...
    #     df_val.loc[t]["number_of_tube_columns"] = int(m.fs.aPSH.tube_ncol.value)
    #     df_val.loc[t]["number_of_inlet_tube_row"] = int(m.fs.aPSH.tube_inlet_nrow.value)
    ...
    #
    #     df_val.to_csv(f"SH_multiobj_case0.csv")

    print('tube length segment', pyo.value(m.fs.aPSH.tube_length_seg))
    print('number of tube on each column', pyo.value(m.fs.aPSH.tube_ncol))
    print('number of rows at inlet', pyo.value(m.fs.aPSH.tube_inlet_nrow))
    print('tube thickness', pyo.value(m.fs.aPSH.tube_thickness))
    print('inside diameter', pyo.value(m.fs.aPSH.tube_di))
    print('pitch y', pyo.value(m.fs.aPSH.pitch_y))
    print('minimumum metal mass', metal_mass_opt)
    print('pressure drop', pyo.value(pressure_drop_opt))
    print('stress range', pyo.value(delta_sigma))
    print('max allowbale cycles', max_allowable_cycles)
    print('multi-objective function values,', pyo.value(multi_obj_f))
...
def write_data_to_txt_file(plot_data):
    ...
    icount = 0
    with open("result_SH_multiobj_case9.txt","w") as fout:
        # write headings
        for k in plot_data:
            icount += 1
            fout.write(k)
