...
tube side flows from 1 to 0"""))
    ...
    CONFIG.declare("estimate_allowable_cycles", ConfigValue(
        default=True,
        domain=In([True, False]),
        description="Flag to include maximum allowable cycles",
        doc="""If estimate_allowable_cycles is True, user can calculate allowable cycles."""))
    ...
    def _make_geometry(self):
        ...
        self.delta_elevation = Var(initialize=0.0,
                                   doc='Elevation Increase Used for'
        ...
        self.tube_nseg = Var(initialize=10.0,
                             doc='Number of Tube Segments')

        # Number of inlet tube rows
        self.tube_inlet_nrow = Var(initialize=1,
                                   doc='Number of Inlet Tube Rows')

        # DAE discretization scaling (tube_radius_scaling)
        self.scaled_tube_thickness = Param(initialize=100, mutable=True,
                                doc='scaled tube thickness for distritization') 
        ...
        if self.config.has_header is True:
            self.head_di = Param(initialize=self.config.header_inner_diameter,
                                 mutable=False,
                                 doc='Inner Diameter of Tube')

            # Thickness of header
            self.head_thickness = Param(initialize=self.config.
        ...
        self.tube_length_seg = Var(initialize=1.0,
        ...
        # radius direction scaling factor
        @self.Expression(doc="Scaling factor in radius direction of tube")
        def r_scaling(b):
            return b.tube_thickness / b.scaled_tube_thickness
        ...
        if self.config.has_radiation is True:
            ...
            def mbl(b):
                return 3.6*(b.pitch_x*b.pitch_y/const.pi/b.tube_do
                            - b.tube_do/4.0)
        ...
        @self.Expression(doc="Ratio of Pitch in x Direction"
                         "to Tube Outside Diameter")
        def pitch_x_to_do(b):
            return b.pitch_x / b.tube_do

        # Ratio of pitch_y/tube_do
        @self.Expression(doc="Ratio of Pitch in y Direction "
                         "to Tube Outside Diameter")
        def pitch_y_to_do(b):
            return b.pitch_y / b.tube_do

        # Total cross section area of tube metal per segment
        @self.Expression(doc="Total Cross Section Area of"
                         "Tube Metal Per Segment")
        def area_wall_seg(b):
            return 0.25*const.pi*(b.tube_do**2
                                  - b.tube_di**2)*b.tube_ncol*b.tube_inlet_nrow
        ...
        def area_flow_tube_eqn(b):
            return b.area_flow_tube == 0.25 * const.pi * b.tube_di**2.0 \
                * b.tube_ncol * b.tube_inlet_nrow
        ...
        def area_flow_shell_eqn(b):
            return b.length_flow_shell*b.area_flow_shell == \
                b.tube_length_seg*b.length_flow_shell \
                * b.pitch_y*b.tube_ncol - b.tube_ncol*b.tube_nrow*0.25 \
                * const.pi*b.tube_do**2*b.tube_length_seg

        # Minimum flow area on shell side
        @self.Constraint(doc="Minimum Flow Area on Shell Side")
        def area_flow_shell_min_eqn(b):
            return b.area_flow_shell_min == b.tube_length_seg\
                * (b.pitch_y-b.tube_do)*b.tube_ncol
        ...
        self.r = ContinuousSet(bounds=(0,
                                       value(self.scaled_tube_thickness)))
    ...
    def _make_performance(self):
        ...
        # Wall density
        self.dens_wall = Param(initialize=7800.0, mutable=True,
                               doc='Tube Wall Density')

        # Young modulus
        self.Young_modulus = Param(initialize=1.90E5, mutable=True,
                                   doc='Tube Wall Young Modulus')

        # Poisson's ratio
        self.Poisson_ratio = Param(initialize=0.29, mutable=True,
                                   doc='Tube Wall Poisson Ratio')

        # Coefficient of thermal expansion
        self.coefficient_thermal_expansion = Param(initialize=1.2E-5,
                                                   mutable=True,
                                                   doc='Tube Wall Coefficient'
        ...
        def hconv_tube_foul_eqn(b, t, x):
            return 0.01 * b.hconv_tube_foul[t, x] * \
        ...
        def hconv_shell_foul_eqn(b, t, x):
            return 0.1*b.hconv_shell_foul[t, x] \
        ...
        def temp_wall_shell_eqn(b, t, x):
            return b.shell_wall_temperature[t, x] == \
        ...
        discretizer.apply_to(self, nfe=self.config.radial_elements,
        ...
        def heat_conduction_eqn(b, t, x, r):
            ...
            if self.config.dynamic is True:
                return b.dTdt[t, x, r]*b.r_scaling**2*(b.tube_ri+r*b.r_scaling) == \
                b.diff_therm_wall*(b.d2Tdr2[t, x, r]*(b.tube_ri+r*b.r_scaling) + b.dTdr[t, x, r]*b.r_scaling) #revise by dT/dt = alpha*(d2T/dr2 + 1/r*dT/dr)
            else:
                return 0 == \
                b.diff_therm_wall*(b.d2Tdr2[t, x, r]*(b.tube_ri+r*b.r_scaling) + b.dTdr[t, x, r]*b.r_scaling)
        ...
        def inner_wall_bc_eqn(b, t, x):
            return 0.01*b.hconv_tube_foul[t, x] \
                * (b.tube.properties[t, x].temperature
                ...
                - 0.01*b.dTdr[t, x, b.r.first()]/b.r_scaling \
                * b.therm_cond_wall
        ...
        def outer_wall_bc_eqn(b, t, x):
            return 0.01 * b.hconv_shell_foul[t, x]\
                * (b.tube_wall_temperature[t, x, b.r.last()]
                   - b.shell.properties[t, x].temperature) == \
                - 0.01*b.dTdr[t, x, b.r.last()]/b.r_scaling*b.therm_cond_wall
        ...
        def extra_at_inner_wall_eqn(b, t, x):
            if self.config.dynamic is True:
                ...
                term = 0
            return term*(4*b.tube_ri+b.r[2]*b.r_scaling)*(b.r[2]*b.r_scaling) == \
                4 * b.diff_therm_wall * (b.tube_ri*2+b.r[2]*b.r_scaling)\
                /(b.r[2]*b.r_scaling)\
                * (b.tube_wall_temperature[t, x, b.r[2]]
                   - b.tube_wall_temperature[t, x, b.r.first()]) \
                + 8*b.diff_therm_wall/b.therm_cond_wall \
                * b.hconv_tube_foul[t, x] * b.tube_ri \
                *(b.tube.properties[t, x].temperature
                                - b.tube_wall_temperature[t, x, b.r.first()])
        ...
        def extra_at_outer_wall_eqn(b, t, x):
            if self.config.dynamic is True:
                ...
                term = 0
            return term*(b.r.last() - b.r[-2])*b.r_scaling*(4*b.tube_ri+(3*b.r.last()+b.r[-2])*b.r_scaling) == \
                4 * b.diff_therm_wall * ((b.r.last() + b.r[-2])*b.r_scaling + 2*b.tube_ri)\
                / ((b.r.last() - b.r[-2])*b.r_scaling)\
                * (b.tube_wall_temperature[t, x, b.r[-2]]
                   - b.tube_wall_temperature[t, x, b.r.last()]) \
                + 8*b.diff_therm_wall/b.therm_cond_wall \
                * b.hconv_shell_foul[t, x] * b.tube_ro\
                * (b.shell.properties[t, x].temperature
                   - b.tube_wall_temperature[t, x, b.r.last()])
        ...
        if self.config.has_radiation is True:
            ...
            def gas_gray_fraction_eqn(b, t, x):
                return b.gas_gray_fraction[t, x] \
                    * (2*b.gas_emissivity_div2[t, x]
            ...
            def hconv_shell_rad_eqn(b, t, x):
                return b.hconv_shell_rad[t, x] == const.stefan_constant \
                    * b.frad_gas_shell[t, x] * (b.shell.properties[t, x].
                                                temperature
                                                + b.
                                                shell_wall_temperature[t, x]) \
        ...
        def v_tube_eqn(b, t, x):
            return 0.001 * b.tube_velocity[t, x] * b.area_flow_tube * \
        ...
        def N_Re_tube_eqn(b, t, x):
            return b.tube_N_Re[t, x] \
                * b.tube.properties[t, x].visc_d_phase[phase_s] == \
                b.tube_di * b.tube_velocity[t, x] \
        ...
        def friction_factor_tube_eqn(b, t, x):
            return b.tube_friction_factor[t, x] * b.tube_N_Re[t, x]**0.25 == \
        ...
        def deltaP_tube_friction_eqn(b, t, x):
            return b.deltaP_tube_friction[t, x] * b.tube_di == \
                - 0.5 * b.tube.properties[t, x].dens_mass_phase[phase_s] \
        ...
        def deltaP_tube_uturn_eqn(b, t, x):
            return b.deltaP_tube_uturn[t, x] * b.tube_length_seg == \
                - 0.5 * b.tube.properties[t, x].dens_mass_phase[phase_s] \
        ...
        def deltaP_tube_eqn(b, t, x):
            return b.tube.deltaP[t, x] == \
                (b.deltaP_tube_friction[t, x] + b.deltaP_tube_uturn[t, x]
                 - b.delta_elevation/b.tube_nseg * const.acceleration_gravity
                 * b.tube.properties[t, x].dens_mass_phase[phase_s] /
                 b.tube_length_seg)
        ...
        def N_Nu_tube_eqn(b, t, x):
            return b.tube_N_Nu[t, x] == 0.023 * b.tube_N_Re[t, x]**0.8 \
                * b.tube_N_Pr[t, x]**0.4
        ...
        def hconv_tube_eqn(b, t, x):
            return b.hconv_tube[t, x]*self.tube_di == b.tube_N_Nu[t, x] * \
                b.tube.properties[t, x].therm_cond_phase[phase_s] \
        ...
        def N_Re_shell_eqn(b, t, x):
            return b.shell_N_Re[t, x] * b.shell.properties[t, x].visc_d == \
                b.tube_do * b.shell_velocity[t, x] \
                * b.shell.properties[t, x].dens_mol_phase["Vap"] *\
        ...
        if self.config.tube_arrangement == "in-line":
            ...
            def friction_factor_shell_eqn(b, t, x):
                return b.shell_friction_factor[t, x] \
                    * b.shell_N_Re[t, x]**0.15 == \
                    (0.044 + 0.08 * b.pitch_x_to_do
                     / (b.pitch_y_to_do - 1.0)**(0.43
            ...
            def friction_factor_shell_eqn(b, t, x):
                return b.shell_friction_factor[t, x] \
                    * b.shell_N_Re[t, x]**0.16 == (0.25
                                                   + 0.118 / (b.pitch_y_to_do
                                                              - 1.0)**1.08) \
                    * b.fcorrection_dp_shell
        else:
            raise Exception('Tube Arrangement Not Supported')
        ...
        def deltaP_shell_eqn(b, t, x):
            return b.shell.deltaP[t, x] * b.pitch_x == - 1.4 \
        ...
        def N_Pr_shell_eqn(b, t, x):
            return b.shell_N_Pr[t, x] * b.shell.properties[t, x].therm_cond \
                * b.shell.properties[t, x].mw == \
                b.shell.properties[t, x].cp_mol \
        ...
        def N_Nu_shell_eqn(b, t, x):
            return b.shell_N_Nu[t, x] == b.f_arrangement * 0.33 \
                * b.shell_N_Re[t, x]**0.6 * b.shell_N_Pr[t, x]**0.333333
        ...
        def hconv_shell_conv_eqn(b, t, x):
            return b.hconv_shell_conv[t, x] * b.tube_do == \
                b.shell_N_Nu[t, x] * b.shell.properties[t, x].therm_cond \
        ...
        def heat_tube_eqn(b, t, x):
            return b.tube_heat[t, x] == b.hconv_tube_foul[t, x] * const.pi \
                * b.tube_di * b.tube_inlet_nrow * b.tube_ncol * \
                (b.tube_wall_temperature[t, x, b.r.first()]
                 - b.tube.properties[t, x].temperature)
        ...
        def heat_shell_eqn(b, t, x):
            return b.shell_heat[t, x] * b.length_flow_shell == \
                b.length_flow_tube*b.hconv_shell_foul[t, x] * const.pi \
                * b.tube_do * b.tube_inlet_nrow * b.tube_ncol * \
                (b.tube_wall_temperature[t, x, b.r.last()]
                 - b.shell.properties[t, x].temperature)
        ...
        # revise by r = rin + r*scaling_factor
        @self.Expression(self.flowsheet().config.time,
                         self.tube.length_domain,
                         doc="Mean Temperature across the Wall")
        def mean_temperature(b, t, x):
            return  (b.r[2]-b.r[1]) * b.r_scaling / (b.tube_ro**2
                                                            - b.tube_ri**2) * \
                (sum(b.tube_ri*(b.tube_wall_temperature[t, x, b.r[i-1]] + \
                    b.tube_wall_temperature[t, x, b.r[i]]) + b.r_scaling * (
                    b.r[i-1]* b.tube_wall_temperature[t, x, b.r[i-1]]
        ...
        def discrete_mean_temperature(b, t, x, r):
            if b.rindex[r].value == 1:
                ...
                return (b.r[2] - b.r[1]) / (b.r[b.rindex[r].value]*(2*b.tube_ri + b.r[b.rindex[r].value] * b.r_scaling)) * \
                    (sum(b.tube_ri * (b.tube_wall_temperature[t, x, b.r[j-1]] + b.tube_wall_temperature[t, x, b.r[j]]
                        ) + b.r_scaling * (b.r[j-1] *
        ...
        def therm_sigma_r(b, t, x, r):
            r_2 = (b.tube_ri + r*b.r_scaling)**2
            if r == b.r.first() or r == b.r.last():
                return 0
            else:
                return 0.5 * b.Young_modulus \
                    * b.coefficient_thermal_expansion / (1 - b.Poisson_ratio) \
                    * ((1 - b.tube_ri**2 / r_2) *
        ...
        @self.Expression(self.flowsheet().config.time,
                         ...
                         doc="Thermal Stress at "
                             "Circumferential Direction for Tube")
        ...
        def therm_sigma_z(b, t, x, r):
            return b.Young_modulus * b.coefficient_thermal_expansion \
        ...
        def mech_sigma_r(b, t, x, r):
            ...
            if r == b.r.first():
                ...
                return 0.1 * (1E-5*(b.tube.properties[t, x].pressure *
                                    b.tube_ri**2 -
                                    b.shell.properties[t, x].pressure *
                                    b.tube_ro**2) /
                              (b.tube_ro**2 -
                               b.tube_ri**2) +
                              (1E-5 * (b.shell.properties[t, x].pressure
                               - b.tube.properties[t, x].pressure) *
                               b.tube_ri**2 * b.tube_ro**2 /
                               (r_2 *
        ...
        def mech_sigma_z(b, t, x):
            return 0.1*(1E-5*(b.tube.properties[t, x].pressure * b.tube_ri**2
                              - b.shell.properties[t, x].pressure
                              * b.tube_ro**2)
                        / (b.tube_ro**2 - b.tube_ri**2))
        ...
        @self.Expression(self.flowsheet().config.time,
                         ...
                         self.r,
                         doc="Principal Structural Stress"
                         "at Circumferential Direction for Tube")
        ...
        @self.Expression(self.flowsheet().config.time,
                         ...
                         self.r,
                         doc='Variation Principal Stress'
                         'between Axial-Radial Directions for Tube')
        ...
        # # --------------------------------------------------------------------------
        # this section is used to estimate number of maximum allowable cyles EN13445
        if self.config.estimate_allowable_cycles is True:
            self.tube_max_stress = Var(self.flowsheet().config.time,initialize=80.0)
            self.tube_min_stress = Var(self.flowsheet().config.time,initialize=73.83)

            @self.Constraint(self.flowsheet().config.time,
                              doc="maximum SH tube wall stress")
            def cycle_max_stress_eqn(b, t):
                if t == b.flowsheet().config.time.first():
                    return 1e-2*b.tube_max_stress[t] == 1e-2*b.sigma_von_Mises[t, 0, b.r.first()]
                else:
                    t_prev = b.flowsheet().config.time.prev(t)
                    return 1e-2*b.tube_max_stress[t] == 1e-2*smooth_max(
                    b.tube_max_stress[t_prev], b.sigma_von_Mises[t, 0, b.r.first()])

            @self.Constraint(self.flowsheet().config.time,
                          doc="minimum SH tube wall stress")
            def cycle_min_stress_eqn(b, t):
                if t == b.flowsheet().config.time.first():
                    return 1e-2*b.tube_min_stress[t] == 1e-2*b.sigma_von_Mises[t, 0, b.r.first()]
                else:
                    t_prev = b.flowsheet().config.time.prev(t)
                    return 1e-2*b.tube_min_stress[t] == 1e-2*smooth_min(
                    b.tube_min_stress[t_prev], b.sigma_von_Mises[t, 0, b.r.first()])

            @self.Expression(self.flowsheet().config.time)
            def delta_sigma(b, t):
                return b.tube_max_stress[t] - b.tube_min_stress[t]

            @self.Expression(self.flowsheet().config.time)
            def mean_sigma(b, t):
                return 0.5 * (b.tube_max_stress[t] + b.tube_min_stress[t])

            # define variable to calculate maximum, minimum temperature of superheater
            self.tube_max_temperature = Var(self.flowsheet().config.time,initialize=724.89) #736.92
            self.tube_min_temperature = Var(self.flowsheet().config.time,initialize=724.89) # 705.16
            @self.Constraint(self.flowsheet().config.time,
                              doc="maximum SH tube wall temperature")
            def max_temperature_eqn(b, t):
                if t == b.flowsheet().config.time.first():
                    return 1e-2*b.tube_max_temperature[t] == 1e-2*b.tube_wall_temperature[t, 0, b.r.first()]
                else:
                    t_prev = b.flowsheet().config.time.prev(t)
                    return 1e-2*b.tube_max_temperature[t] == 1e-2*smooth_max(
                    b.tube_max_temperature[t_prev], b.tube_wall_temperature[t, 0, b.r.first()])

            @self.Constraint(self.flowsheet().config.time,
                          doc="minimum SH tube wall temperature")
            def min_temperature_eqn(b, t):
                if t == b.flowsheet().config.time.first():
                    return 1e-2*b.tube_min_temperature[t] == 1e-2*b.tube_wall_temperature[t, 0, b.r.first()]
                else:
                    t_prev = b.flowsheet().config.time.prev(t)
                    return 1e-2*b.tube_min_temperature[t] == 1e-2*smooth_min(
                    b.tube_min_temperature[t_prev], b.tube_wall_temperature[t, 0, b.r.first()])

            self.delta_sigma_D = Param(default=273.0,mutable=True)  # MPa
            self.Rm = Param(default=380, mutable=True)  # MPa
            self.Rp = Param(default=205, mutable=True)  # MPa
            self.K_t = Param(default=3.0, mutable=True)
            self.Rz = Param(default=50, mutable=True)

            @self.Expression(self.flowsheet().config.time)
            def sigma_max(b, t):
                return b.mean_sigma[t] + 0.5 * b.delta_sigma[t]

            if value(self.Rm) <= 500:
                A0 = 0.4 # for Rm <= 500 MPa and for all austenitic steels
            elif (value(self.Rm)>500 and value(self.Rm) <=800):
                A0 = 0.4+(self.Rm-500)/3000
            elif (value(self.Rm)>800 and value(self.Rm) <=1000):
                A0 = 0.5

            @self.Expression(self.flowsheet().config.time)
            def k_e(b, t):
                return Expr_if(value(b.delta_sigma[t]) > value(2*b.Rp), 1 + A0*(b.delta_sigma[t]/(2*b.Rp)-1),1)
            ...
            def k_v(b, t):
                if t == b.flowsheet().config.time.first():
                    ...
                    return Expr_if(value(b.delta_sigma[t]) > value(2*b.Rp), max(1, value(0.7/(0.5+0.4/(b.delta_sigma[t]/b.Rp)))), 1)

            @self.Expression(self.flowsheet().config.time)
            def delta_sigma_eq(b, t):
                return b.delta_sigma[t] * b.k_e[t] * b.k_v[t]

            @self.Expression(self.flowsheet().config.time)
            def f_T(b, t):
                T_max = b.tube_max_temperature[t] -273.15
                T_min = b.tube_min_temperature[t] -273.15
                return 1.03 - 1.5e-4*(0.75*T_max+0.25*T_min
                    )-1.5e-6*(0.75*T_max+0.25*T_min)**2

            self.delta_sigma_R = Var(self.flowsheet().config.time, initialize=140.0, domain=NonNegativeReals)
            self.allowable_cycles = Var(self.flowsheet().config.time, initialize=7.7e8, domain=NonNegativeReals)

            @self.Expression(self.flowsheet().config.time)
            def K_f(b, t):
                return Expr_if(value(b.allowable_cycles[t])<=2e6, 1+1.5*(b.K_t-1)/(
                    1+0.5*max(1,(value(b.K_t*b.delta_sigma_eq[t]/b.delta_sigma_D)))), 1+1.5*(b.K_t-1)/(
                    1+0.5*max(1,(value(b.K_t*b.delta_sigma_eq[t]/b.delta_sigma_R[t])))))

            @self.Expression(self.flowsheet().config.time)
            def delta_sigma_f(b, t):
                return b.K_f[t] * b.delta_sigma_eq[t]

            @self.Expression(self.flowsheet().config.time)
            def f_s(b, t):
                return Expr_if(value(b.allowable_cycles[t])<=2e6, (1 - 0.056*(
                    log(b.Rz))**0.64 * log(b.Rm) + 0.289*(log(b.Rz))**0.53)**(
                    0.1*(log(b.allowable_cycles[t]))-0.465), 1 - 0.056*(
                    log(b.Rz))**0.64 * log(b.Rm) + 0.289*(log(b.Rz))**0.53)
            
            @self.Expression(self.flowsheet().config.time)
            def f_e(b, t):
                RI = 0.5*b.tube_di
                RO = 0.5*b.tube_di + b.tube_thickness
                e_n = (RO-RI)*1000
                if value(e_n) <25:
                    return 1
                elif (value(e_n) > 25 and value(b.allowable_cycles[t]) <= 2e6):
                    return Expr_if((value(e_n)<=150 and value(e_n)>25), ((
                        25/value(e_n))**0.182)**(0.1*(log(b.allowable_cycles[t]))-0.465), (1/6)**0.182)
                elif (value(e_n) > 25 and value(b.allowable_cycles[t]) > 2e6):
                    return Expr_if((value(e_n)<=150 and value(e_n)>25), (25/value(e_n))**0.182, (1/6)**0.182)

            @self.Expression(self.flowsheet().config.time)
            def sigma_mean_r(b, t):
                if inequality(value(b.delta_sigma[t]),value(2*b.Rp)) and inequality(value(b.Rp),value(abs(b.sigma_max[t]))): #1e-4*smooth_abs(b.sigma_max[t])
                    return Expr_if(value(b.mean_sigma[t])>0, b.Rp - 0.5*b.mean_sigma[t], 0.5*b.mean_sigma[t] - b.Rp)
                elif inequality(value(b.delta_sigma[t]),value(2*b.Rp)) and inequality(value(abs(b.sigma_max[t])),value(b.Rp)): #1e-4*smooth_abs(b.sigma_max[t])
                    return b.mean_sigma[t]
                else:
                    return 0

            M=0.00035*self.Rm-0.1

            @self.Expression(self.flowsheet().config.time)
            def A1(b, t):
                return b.delta_sigma_R[t]/(2*(1+M))

            @self.Expression(self.flowsheet().config.time)
            def f_m(b, t):
                if value(b.allowable_cycles[t])<=2e6:
                    if inequality(-b.Rp,value(b.sigma_mean_r[t]),value(b.A1[t])): #(-b.Rp <= value(b.sigma_mean_r[t]) and value(b.sigma_mean_r[t]) <= value(b.A1[t]))
                        return sqrt(1-M*(2+M)/(1+M)*(2*b.sigma_mean_r[t]/b.delta_sigma_R[t]))
                    elif inequality(value(b.A1[t]),value(b.sigma_mean_r[t]),b.Rp):
                        return (1+M/3)/(1+M) - M/3*(2*b.sigma_mean_r[t]/b.delta_sigma_R[t])
                    else:
                        return 1
                else:
                    if inequality(-b.Rp,value(b.sigma_mean_r[t]),value(b.A1[t])):
                        return sqrt(1-M*(2+M)/(1+M)*(2*b.sigma_mean_r[t]/b.delta_sigma_D))
                    elif inequality(value(b.A1[t]),value(b.sigma_mean_r[t]),b.Rp):
                        return (1+M/3)/(1+M) - M/3*(2*b.sigma_mean_r[t]/b.delta_sigma_D)
                    else:
                        return 1

            @self.Expression(self.flowsheet().config.time)
            def f_u(b, t):
                return b.f_T[t] * b.f_s[t] * b.f_e[t] * b.f_m[t]

            @self.Constraint(self.flowsheet().config.time)
            def delta_sigma_R_cons(b, t):
                return 1e-4*b.delta_sigma_R[t] == 1e-4*b.delta_sigma_f[t]/b.f_u[t]

            @self.Constraint(self.flowsheet().config.time)
            def N_cons(b, t):
                if inequality(value(b.delta_sigma_D),value(b.delta_sigma_R[t])):
                    return b.allowable_cycles[t] == (
                    46000/(b.delta_sigma_R[t]-0.63*b.Rm + 11.5))**2
                else:
                    return log10(b.allowable_cycles[t]) == 10*log10(
                        (2.69*b.Rm+89.72)/b.delta_sigma_R[t])
                # return 1e-10*b.allowable_cycles[t] == 1e-10*Expr_if(value(b.delta_sigma_R[t]) > b.delta_sigma_D, (
                #     46000/(b.delta_sigma_R[t]-0.63*b.Rm + 11.5))**2,((2.69*b.Rm+89.72)/b.delta_sigma_R[t])**10)
        ...
        if self.config.has_header is True:
            ...
            # Coefficient of thermal expansion
            self.coefficient_therm_expansion_header = Param(initialize=1.2E-5,
                                                            mutable=True)
            ...
            self.head_therm_cond_insulation = Param(initialize=0.08,
                                                    mutable=True)
            ...
            self.head_insulation_thickness = Var(
                                       initialize=0.05,
                                       doc="Header insulation thickness")
            ...
            # Declare derivatives in the model
            if self.config.dynamic is True:
                self.head_dTdt = DerivativeVar(self.header_wall_temperature,
                                               wrt=self.flowsheet().config.
                                               time)
            self.head_dTdr = DerivativeVar(self.header_wall_temperature,
                                           wrt=self.head_r)
            self.head_d2Tdr2 = DerivativeVar(self.header_wall_temperature,
                                             wrt=(self.head_r, self.head_r))

            discretizer_2 = TransformationFactory('dae.finite_difference')
            discretizer_2.apply_to(self,
                                   nfe=self.config.header_radial_elements,
                                   wrt=self.head_r, scheme='CENTRAL')
            ...
            def head_outer_wall_bc_eqn(b, t):
                return 0.01 * b.head_hout[t] * \
                    (b.header_wall_temperature[t, b.head_r.last()] -
                     b.temperature_ambient[t]) == - 0.01 * \
                    b.head_dTdr[t, b.head_r.last()] / b.head_ri_scaling \
                    * b.therm_cond_header
            ...
            def resistance_insulation(b):
                return b.head_ro*log((b.head_ro + b.head_insulation_thickness)
                                     / b.head_ro)/b.head_therm_cond_insulation
            ...
            def head_temp_insulation_outside(b, t):
                return b.temperature_ambient[t] + (
                    b.header_wall_temperature[t, b.head_r.last()]
                    - b.temperature_ambient[t]) * b.head_hout[t]\
            ...
            def head_Ra_number_eqn(b, t):
                return b.head_N_Ra_root6[t] == b.const_head_Ra_root6 * sqrt(
                    b.head_do + 2 * b.head_insulation_thickness) * (
                        b.head_temp_insulation_outside[t]
            ...
            def head_h_free_conv_eqn(b, t):
                return b.head_h_free_conv[t] == b.head_N_Nu_shell[t] \
            ...
            def head_velocity_tube_eqn(b, t):
                return (0.0001 * b.head_velocity_tube[t] * const.pi *
                        b.head_ri**2 *
                        b.tube.properties[t, b.tube.length_domain.first()].
                        dens_mol_phase[phase_s] == 0.0001 * b.tube.
            ...
            def head_N_Re_tube_eqn(b, t):
                return (b.head_N_Re_tube[t]
                        * b.tube.properties[t, b.tube.length_domain.first()].
                        visc_d_phase[phase_s] ==
                        b.head_di * b.head_velocity_tube[t]
                        * b.tube.properties[t, b.tube.length_domain.first()].
            ...
            # ------------------------------------------------------------
            # Integer indexing for radius domain
            self.head_rindex = Param(self.head_r,
            ...
            def therm_sigma_r_header(b, t, r):
                if r == b.head_r.first() or r == b.head_r.last():
                    ...
                    return 0.5 * b.Young_modulus_header \
                ...
                if r == b.head_r.first():
                    return 1e-6 * (-b.
                                   ...
                                   properties[t,
                                              b.
                                              tube.
                                              length_domain.first()].pressure)
                elif r == b.head_r.last():
                    return 1e-6 * (-b.
                                   shell.
                                   properties[t,
                                              b.
                                              shell.length_domain.first()].
                                   pressure)
                else:
                    return 0.1 * (1E-5 * (b.tube.properties[t, b.
                                                            tube.
                                                            length_domain.
                                                            first()].
                                          pressure * b.head_ri**2 - b.
                                          shell.
                                          properties[t,
                                                     b.
                                                     shell.
                                                     length_domain.first()].
                                          pressure * b.head_ro**2)
                                  / (b.head_ro**2 - b.head_ri**2)
                                  + (1E-5 * (b.
                                             shell.
                                             properties[t, b.
            ...
            def sigma_von_Mises_header(b, t, r):
                return sqrt(b.sigma_r_header[t, r]**2
                            ...
                            - (b.sigma_r_header[t, r]
                               * b.sigma_theta_header[t, r]
                               + b.sigma_r_header[t, r]
                               * b.sigma_z_header[t, r]
                               + b.sigma_theta_header[t, r]
                               * b.sigma_z_header[t, r]))
            ...
            k_m_A = -1.14 * (pipe_th / header_thickness)**2 \
                - 0.89 * (pipe_th / header_thickness) + 1.43
            k_m_B = 0.326 * (pipe_th / header_thickness)**2 \
                - 0.59 * (pipe_th / header_thickness) + 1.08
            k_m_C = pipe_d / (2 * r_ms_head) * sqrt((2 * r_ms_head)
                                                    / (2 * header_thickness))
            ...
            k_t_B = 1000
            k_t_header = sqrt((2 - (k_t_B + 2700) / (k_t_B + 1700)
                               * k_t_A + k_t_B / (k_t_B + 1700)
            ...
            def sigma_t(b, t):
                delta_T = b.mean_temperature_header[t] \
                            - b.header_wall_temperature[t, b.head_r.first()]
                return b.coefficient_therm_expansion_header \
                    * b.Young_modulus_header / (1 - b.Poisson_ratio_header) \
                    * delta_T
            ...
            def sigma_eff_P1(b, t):
                ...
                return sqrt(sigma_comer_P1**2 + (-1e-6 * p_in)**2
                            + (-1e-6 * p_in)**2 - ((-1E-6 * p_in)
                            * sigma_comer_P1 + (-1e-6 * p_in)
                        * sigma_comer_P1 + (-1e-6 * p_in)
                        * (-1e-6 * p_in)))
            ...
            def sigma_eff_P2(b, t):
                ...
                p_in = \
                    b.tube.properties[t, b.tube.length_domain.first()].pressure
                return sqrt(sigma_comer_P2**2 + (-1e-6 * p_in)**2
                            + (-1e-6*p_in)**2 - ((-1E-6 * p_in)
                            * sigma_comer_P2 + (-1e-6 * p_in)
                        * sigma_comer_P2 + (-1e-6 * p_in)
                        * (-1e-6 * p_in)))
            ...
            def rupture_time_crotch_corner(b, t):                
                return Expr_if(b.sigma_eff_P1[t] > 10, 10**((b.creep_a_header + b.creep_b_header
                             * log10(b.sigma_eff_P1[t])
                             + b.creep_c_header
                             * (log10(b.sigma_eff_P1[t]))**2
                             + b.creep_d_header
                             * (log10(b.sigma_eff_P1[t]))**3)
                            * (b.header_wall_temperature[t,
                                                         b.head_r.first()]
                               - b.creep_f_header)
                            + b.creep_e_header),1e13)
            ...
            def rupture_time_P2(b, t):
                return Expr_if(b.sigma_eff_P2[t] > 10,10**((b.creep_a_header + b.creep_b_header
                             * log10(b.sigma_eff_P2[t])
                             + b.creep_c_header
                             * (log10(b.sigma_eff_P2[t]))**2
                             + b.creep_d_header
                             * (log10(b.sigma_eff_P2[t]))**3)
                            * (b.header_wall_temperature[t,
                                                         b.head_r.first()]
                               - b.creep_f_header)
                            + b.creep_e_header),1e13)
    ...
    def initialize(blk, shell_state_args=None, tube_state_args=None,
        ...
        if blk.config.flow_type == "co_current":
            mcp_shell = value(blk.shell.properties[0, 0].flow_mol
                              * blk.shell.properties[0, 0].cp_mol)
            mcp_tube = value(blk.
                             tube_inlet.
                             flow_mol[0] * blk.
                             tube.
                             properties[0, 0].cp_mol_phase[phase_s])
            tout_max = (mcp_tube * value(blk.
                                         tube.
                                         properties[0, 0].
                                         temperature
                                         ) + mcp_shell * value(
                                             blk.shell.properties[0, 0].
                                             temperature)) / (mcp_tube
                                                              + mcp_shell)
            q_guess = mcp_tube * value(
                tout_max - value(blk.tube.properties[0, 0].temperature))/2
            temp_out_tube_guess = value(blk.
                                        tube.
                                        properties[0, 0].
                                        temperature) + q_guess / mcp_tube
            temp_out_shell_guess = value(
                blk.shell.properties[0, 0].temperature) - q_guess / mcp_shell
            if phase_s == 'Liq' and temp_out_tube_guess > value(
                                    blk.tube.properties[0, 0].temperature_sat):
                init_log.info("Estimated Outlet Liquid Water Temperature"
                              "Exceeds the Saturation Temperature.")
                init_log.info("Estimated Outlet"
                              " Liquid Water Temperature = {}.".format(
                                  temp_out_tube_guess))
                init_log.info("Saturation Temperature at"
                              " Inlet Pressure = {}.".format(
                                  value(blk.tube.properties[0, 0].
                                        temperature_sat)))
                temp_out_tube_guess = value(
                    0.9 * blk.tube.properties[0, 0].temperature_sat
                    + 0.1*blk.tube.properties[0, 0].temperature)
                init_log.info("Reset Estimated Outlet Liquid "
                              "Water Ttemperature = {}.".format(
                                  temp_out_tube_guess))
        else:
            mcp_shell = value(blk.shell.properties[0, 0].flow_mol
                              * blk.shell.properties[0, 0].cp_mol)
            mcp_tube = value(blk.tube_inlet.flow_mol[0] * blk.
                             tube.properties[0, 1].cp_mol_phase[phase_s])
            if mcp_tube < mcp_shell:
                q_guess = mcp_tube * value(
                    blk.shell.properties[0, 0].temperature
                    - blk.tube.properties[0, 1].temperature) / 2
            else:
                q_guess = mcp_shell * value(
                    blk.shell.properties[0, 0].temperature
                    - blk.tube.properties[0, 1].temperature) / 2
            temp_out_tube_guess = value(
                                    blk.
            ...
            if phase_s == 'Liq' and temp_out_tube_guess > value(
                ...
                init_log.info("Estimated Outlet Liquid Water Temperature"
                              " Exceeds the Saturation temperature.")
                init_log.info("Estimated Outlet Liquid Water "
                              "Temperature = {}.".format(
                                  temp_out_tube_guess))
                init_log.info("Saturation Temperature at Inlet"
                              " Pressure = {}.".format(
                                  value(blk.tube.properties[0, 1].
                                        temperature_sat)))
                temp_out_tube_guess = value(0.9 * blk.
                                            tube.
                                            properties[0, 1].
                                            temperature_sat + 0.1 * blk.
                                            tube.
                                            properties[0, 1].
                                            temperature)
                init_log.info("Reset Estimated Outlet Liquid Water"
                              " Temperature = {}.".format(
                                  temp_out_tube_guess))
                ...
                if blk.config.flow_type == "co_current":
                    blk.tube_wall_temperature[t, z, :].fix(value(
                        0.05 * ((1 - z) * blk.
                                shell.
                                properties[0, 0].
                                temperature + z * temp_out_shell_guess
                                ) + 0.95 * (
                                    (1 - z) * blk.
                                    tube.
                                    properties[0, 0].
                                    temperature + z * temp_out_tube_guess)))
                else:
                    blk.tube_wall_temperature[t, z, :].fix(value(
                        0.05 * ((1 - z) * blk.
                                shell.
                                ...
                                temperature + z * temp_out_shell_guess
                                    ...
                                    (1 - z) * temp_out_tube_guess + z * value(blk.
        ...
        for t in blk.flowsheet().config.time:
            for z in blk.shell.length_domain:
                ...
                blk.shell.properties[t, z].pressure.fix(value(
                    blk.shell.properties[0, 0].pressure))
        ...
        if blk.config.has_header is True:
            ...
            blk.header_wall_temperature.fix()

        if blk.config.estimate_allowable_cycles is True:
            blk.cycle_max_stress_eqn.deactivate()
            blk.cycle_min_stress_eqn.deactivate()
            blk.max_temperature_eqn.deactivate()
            blk.min_temperature_eqn.deactivate()
            blk.delta_sigma_R_cons.deactivate()
            blk.N_cons.deactivate()
            blk.tube_max_stress.fix()
            blk.tube_min_stress.fix()
            blk.tube_max_temperature.fix()
            blk.tube_min_temperature.fix()
            blk.delta_sigma_R.fix()
            blk.allowable_cycles.fix()
        ...
        for t in blk.flowsheet().config.time:
            ...
            blk.shell.properties[t, 0].pressure.fix(
                value(blk.shell_inlet.pressure[0]))
        ...
        if blk.config.has_header is True:
            ...
            temp_head_in = value(
                blk.tube.properties[0, blk.tube.length_domain.first()].
                temperature) - 0.1
            temp_head_out = temp_head_in - 0.5
            for x in blk.head_r:
                blk.header_wall_temperature[:, x].value = (
                    temp_head_in + (temp_head_out - temp_head_in) /
                    (blk.head_r.last() - blk.head_r.first()) *
                    (x - blk.head_r.first()))

        if blk.config.estimate_allowable_cycles is True:
            blk.cycle_max_stress_eqn.activate()
            blk.cycle_min_stress_eqn.activate()
            blk.max_temperature_eqn.activate()
            blk.min_temperature_eqn.activate()
            blk.delta_sigma_R_cons.activate()
            blk.N_cons.activate()
            blk.tube_max_stress.unfix()
            blk.tube_min_stress.unfix()
            blk.tube_max_temperature.unfix()
            blk.tube_min_temperature.unfix()
            blk.delta_sigma_R.unfix()
            blk.allowable_cycles.unfix()
