...
tube side flows from 1 to 0"""))
    ...
    CONFIG.declare("tube_inner_diameter", ConfigValue(
        default=None,
        description='Inner diameter of tube',
        doc='User must define inner diameter of tube'))
    CONFIG.declare("tube_thickness", ConfigValue(
        default=None,
    ...
    def build(self):
        ...
        if self.config.tube_inner_diameter is None:
            raise ConfigurationError('User must provide a value for '
                                     'tube_inner_diameter')
    ...
    def _make_geometry(self):
        ...
        # Thickness of header
        self.tube_thickness = Param(initialize=self.config.tube_thickness,
                                    mutable=False,
                                    doc='Tube thickness')
            ...
            def head_heat_conduction_eqn(b, t, r):
                ...
                if self.config.dynamic is True:
                    return b.head_dTdt[t, r] == \
                            b.therm_diffus_header / b.ri_scaling**2 * \
                            (b.head_d2Tdr2[t, r] + b.head_dTdr[t, r] / r)
                else:
                    return 0 == b.therm_diffus_header / b.ri_scaling**2 \
                                * (b.head_d2Tdr2[t, r] + b.head_dTdr[t, r] / r)

            @self.Constraint(self.flowsheet().config.time,
                             doc="Inner Wall Boundary")
            def head_inner_wall_bc_eqn(b, t):
                return 0.01 * b.hconv_tube_foul[t, b.tube.length_domain.
                                                first()] *\
                    ...
                    / b.ri_scaling * b.therm_cond_header

            @self.Constraint(self.flowsheet().config.time,
                             doc="Outer Wall Boundary")
            def head_outer_wall_bc_eqn(b, t):
                return 0.01 * b.hconv_shell_foul[t, b.shell.length_domain.
                                                 first()] * \
                    (b.header_wall_temperature[t, b.head_r.last()] -
                     b.shell.properties[t, b.shell.length_domain.first()].
                     temperature) == - 0.01 * \
                    b.head_dTdr[t, b.head_r.last()] / b.ri_scaling \
                    * b.therm_cond_header
            ...
            def therm_sigma_r_header(b, t, r):
                if r == b.head_r.first() or r == b.head_r.last():
                    ...
                    return 0.5 * b.Young_modulus_header \
                        ...
                        * ((1 - b.head_ri**2 / (r * b.ri_scaling)**2)
    ...
    def initialize(blk, shell_state_args=None, tube_state_args=None,
        ...
        # proposed by JM, set temp_out_tube_guess as initial temperature for header wall temperature
        if blk.config.has_header is True:
            for x in blk.head_r:
                blk.header_wall_temperature[:, x].fix(temp_out_tube_guess+10)
