...
def add_unit_models(m):
    ...
    fs.bfp_turb_valve = SteamValve(default={"dynamic": False,"property_package": prop_water})
    ...
    # Condenser Hotwel, modeled as Mixer. The makeup stream is also added even though it is added to hotwell tank.
    # Set momentum_mixing_type to none since aux_condenser outlet pressure usually not equal to the main condenser outlet pressure
    # We impose the constraints to let the mixed pressure equal to the main condenser pressure and makeup pressure
    ...
    '''
    # water pipe from DA tank to booster pump
    fs.pipe_fw = WaterPipe(default={
                               "dynamic": False,
                               "property_package": prop_water,
                               "has_holdup": True,
    ...
    if m.dynamic==True:
        ...
        fs.da_ctrl = PIDController(default={"pv":fs.da_tank.tank_level,
                                  "mv":fs.cond_valve.valve_opening,
                                  "type": 'PI'})
        # Hotwell makeup water controller
        fs.makeup_ctrl = PIDController(default={"pv":fs.hotwell_tank.tank_level,
...
def set_arcs_and_constraints(m):
    ...
    '''
    fs.S036b = Arc(
        source=fs.da_tank.outlet, destination=fs.pipe_fw.inlet
    )
    fs.S036c = Arc(
        source=fs.pipe_fw.outlet, destination=fs.booster.inlet
    )
    '''
    fs.S038 = Arc(
    ...
    def cond_pump_curve_constraint(b, t):
        return b.deltaP[t]*1e-5 == 1e-5*(-4.6141e-7*b.inlet.flow_mol[t]**3 + 3.7188e-3*b.inlet.flow_mol[t]**2\
                              -51.435*b.inlet.flow_mol[t] + 1.8214e6)
    ...
    def booster_pump_curve_constraint(b, t):
        return b.deltaP[t]*1e-5 == 1e-5*(-2.1494e-7*b.inlet.flow_mol[t]**3 + 2.8681e-3*b.inlet.flow_mol[t]**2\
                              -30.994*b.inlet.flow_mol[t] + 9.9828e5)
...
def set_inputs(m):
    ...
    fs.turb.turbine_outlet_cf_fix(0.0463) #used to be 0.0255
    ...
    fs.turb.hp_split[14].split_fraction[:, "outlet_3"].fix(0.0) #to process plant, used to be 0
    fs.turb.hp_split[14].split_fraction[:, "outlet_4"].fix(0.0)
    fs.turb.ip_split[6].split_fraction[:, "outlet_2"].fix(0.02)
    fs.turb.ip_split[9].split_fraction[:, "outlet_2"].fix(0.01) #set to lower value to avoid flashing of DA, old value 0.02
    ...
    fs.bfp_turb_os.flow_coeff.fix(0.003)  # No longer need to set to small value during initialization
    ...
    # aux condenser
    fs.aux_condenser.inlet_2.flow_mol.fix(11140) # measured around 4200 gpm or 14700 mol/s
    ...
    fs.hotwell_tank.tank_cross_sect_area.fix(84)  # Normal volume 15500 gallons (58.67 m^3)  84*0.7=58.8 m^3
    ...
    fs.fwh1.condense.heater_diameter.fix(1.1684)
    fs.fwh1.condense.vol_frac_shell.fix(0.74)
    fs.fwh1.condense.cond_sect_length.fix(7.6708)
    ...
    fs.fwh2.condense.heater_diameter.fix(1.1938)
    fs.fwh2.condense.vol_frac_shell.fix(0.6923)
    fs.fwh2.condense.cond_sect_length.fix(6.985) # currently use total tube length
    ...
    fs.fwh3.condense.heater_diameter.fix(1.1938)
    fs.fwh3.condense.vol_frac_shell.fix(0.6923)
    fs.fwh3.condense.cond_sect_length.fix(6.9596) # currently use total tube length
    fs.fwh3.condense.tube.volume.fix(1.366)

    fs.fwh3_valve.Cv.value = 1.9257762443933564
    fs.fwh3_valve.valve_opening.fix(0.5)

    fs.da_tank.tank_diameter.fix(3.62585)      # 12 feet OD with 5/8 inch thickness
    fs.da_tank.tank_length.fix(19.812)         # 65 feet
    fs.da_tank.tank_level.fix(2.7432)          # normal level 9 feet from bottom of tank
    '''
    fs.pipe_fw.diameter.fix(0.3556)
    fs.pipe_fw.length.fix(15)
    fs.pipe_fw.count.fix(1);
    fs.pipe_fw.elevation_change.fix(-15)
    '''
    ...
    fs.fwh5.condense.heater_diameter.fix(1.3462)
    fs.fwh5.condense.vol_frac_shell.fix(0.6752)
    fs.fwh5.condense.cond_sect_length.fix(6.1976) # currently use total tube length
    ...
    fs.fwh6.condense.heater_diameter.fix(1.27)
    fs.fwh6.condense.vol_frac_shell.fix(0.6622)
    fs.fwh6.condense.cond_sect_length.fix(8.372) # currently use total tube length
...
def initialize(m):
    ...
    if m.dynamic==True:
        ...
        #fs.pipe_fw.set_initial_condition()
    ...
    '''    
    # pipe_fw initialization
    _set_port(fs.pipe_fw.inlet, fs.da_tank.outlet)
    if m.dynamic==False or m.init_dyn==True:
        fs.pipe_fw.initialize(outlvl=outlvl, optarg=solver.options)
    '''
    # init booster
    #_set_port(fs.booster.inlet, fs.pipe_fw.outlet)
    ...
    if m.dynamic==False or m.init_dyn==True:
        ...
        print ('main steam temp=', pyo.value(fs.turb.throttle_valve[1].control_volume.properties_in[0].temperature))
        print ('main steam enth=', pyo.value(fs.turb.inlet_split.inlet.enth_mol[0]))
        print ('main steam flow_mol=', pyo.value(fs.turb.inlet_split.inlet.flow_mol[0]))
        print ('main steam pressure=', pyo.value(fs.turb.inlet_split.inlet.pressure[0]))
        print ('bfp turb steam flow=', pyo.value(fs.bfp_turb.inlet.flow_mol[0]))
        print ('FW pressure=', pyo.value(fs.bfp.outlet.pressure[0]))
        print ('HP stage 1 inlet enth_mol=', pyo.value(fs.turb.hp_stages[1].inlet.enth_mol[0]))
        print ('HP stage 1 inlet flow_mol=', pyo.value(fs.turb.hp_stages[1].inlet.flow_mol[0]))
        print ('HP stage 1 inlet pressure=', pyo.value(fs.turb.hp_stages[1].inlet.pressure[0]))
        print ('IP stage 1 inlet enth_mol=', pyo.value(fs.turb.ip_stages[1].inlet.enth_mol[0]))
        print ('IP stage 1 inlet flow_mol=', pyo.value(fs.turb.ip_stages[1].inlet.flow_mol[0]))
        print ('IP stage 1 inlet pressure=', pyo.value(fs.turb.ip_stages[1].inlet.pressure[0]))
        print ('Outlet stage enth_mol=', pyo.value(fs.turb.outlet_stage.outlet.enth_mol[0]))
        print ('Outlet stage flow_mol=', pyo.value(fs.turb.outlet_stage.outlet.flow_mol[0]))
        print ('Outlet stage pressure=', pyo.value(fs.turb.outlet_stage.outlet.pressure[0]))
        print ('Power output of main turbine=', pyo.value(fs.power_output[0]))
        print ('Power output of bfp turbine=', pyo.value(fs.bfp_turb.control_volume.work[0]+fs.bfp_turb_os.control_volume.work[0]))
        print ('FWH6 outlet enth_mol=', pyo.value(fs.fwh6.desuperheat.outlet_2.enth_mol[0]))
        print ('FWH6 outlet flow_mol=', pyo.value(fs.fwh6.desuperheat.outlet_2.flow_mol[0]))
        print ('FWH6 outlet pressure=', pyo.value(fs.fwh6.desuperheat.outlet_2.pressure[0]))
        print ('Outlet stage ratioP=', pyo.value(fs.turb.outlet_stage.ratioP[0]))
        print ('Outlet stage inlet P=', pyo.value(fs.turb.outlet_stage.inlet.pressure[0]))
        print ('Outlet stage outlet P=', pyo.value(fs.turb.outlet_stage.outlet.pressure[0]))
        print ('bfp turbine out T=', pyo.value(fs.bfp_turb.control_volume.properties_out[0].temperature))
        print ('bfp turbine out P=', pyo.value(fs.bfp_turb.control_volume.properties_out[0].pressure))
        print ('bfp turbine out H=', pyo.value(fs.bfp_turb.control_volume.properties_out[0].enth_mol))
        print ('bfp turbine out flow=', pyo.value(fs.bfp_turb.control_volume.properties_out[0].flow_mol))
        print ('ip_split9 fraction to DA=', pyo.value(fs.turb.ip_split[9].split_fraction[0,'outlet_2']))
        print ('ip_split9 fraction to bfpt=', pyo.value(fs.turb.ip_split[9].split_fraction[0,'outlet_3']))
        print('booster outlet pres=', fs.booster.outlet.pressure[0].value)
        print('booster inlet temp=', pyo.value(fs.booster.control_volume.properties_in[0].temperature))
        print('booster inlet sat temp=', pyo.value(fs.booster.control_volume.properties_in[0].temperature_sat))
        #check pressure at mixer inlets
        print('hotwell main inlet pressure=', fs.condenser_hotwell.main_condensate.pressure[0].value)
        print('hotwell aux inlet pressure=', fs.condenser_hotwell.aux_condensate.pressure[0].value)
        print('hotwell makeup inlet pressure=', fs.condenser_hotwell.makeup.pressure[0].value)
        print('drain return feedwater inlet pressure=', fs.fwh1_drain_return.feedwater.pressure[0].value)
        print('drain return drain inlet pressure=', fs.fwh1_drain_return.fwh1_drain.pressure[0].value)
        print('DA feedwater inlet pressure=', fs.fwh4_deair.feedwater.pressure[0].value)
        print('DA drain inlet pressure=', fs.fwh4_deair.drain.pressure[0].value)
        print('DA steam inlet pressure=', fs.fwh4_deair.steam.pressure[0].value)
        print('DA outlet pressure=', fs.fwh4_deair.outlet.pressure[0].value)
        print('fwh2 drain pressure before valve=', fs.fwh2_valve.inlet.pressure[0].value)
        print('fwh2 drain pressure after valve=', fs.fwh2_valve.outlet.pressure[0].value)
        print('fwh1 shell pressure =', fs.fwh1.drain_mix.steam.pressure[0].value)
        print('fwh3 drain pressure before valve=', fs.fwh3_valve.inlet.pressure[0].value)
        print('fwh3 drain pressure after valve=', fs.fwh3_valve.outlet.pressure[0].value)
        print('fwh2 shell pressure =', fs.fwh2.drain_mix.steam.pressure[0].value)
        print('fwh5 drain pressure before valve=', fs.fwh5_valve.inlet.pressure[0].value)
        print('fwh5 drain pressure after valve=', fs.fwh5_valve.outlet.pressure[0].value)
        print('fwh6 drain pressure before valve=', fs.fwh6_valve.inlet.pressure[0].value)
        print('fwh6 drain pressure after valve=', fs.fwh6_valve.outlet.pressure[0].value)
        print('fwh5 shell pressure =', fs.fwh5.drain_mix.steam.pressure[0].value)
        #check valve Cv
        print('Cv fwh2=', fs.fwh2_valve.Cv.value)
        print('Cv fwh3=', fs.fwh3_valve.Cv.value)
        print('Cv fwh5=', fs.fwh5_valve.Cv.value)
        print('Cv fwh6=', fs.fwh6_valve.Cv.value)
        print('Cv cond valve=', fs.cond_valve.Cv.value)
        print('Cv hotwell rej valve=', fs.hotwell_rejection_valve.Cv.value)
        print('Cv da rej valve=', fs.da_rejection_valve.Cv.value)
        print('Cv makeup valve=', fs.makeup_valve.Cv.value)
        print('Cv spray valve=', fs.spray_valve.Cv.value)
        print('Cv bfp turb valve=', fs.bfp_turb_valve.Cv.value)
        print('Cv throttle valve=', fs.turb.throttle_valve[1].Cv.value)
        print('valve opening fwh2=', fs.fwh2_valve.valve_opening[0].value)
        print('valve opening fwh3=', fs.fwh3_valve.valve_opening[0].value)
        print('valve opening fwh5=', fs.fwh5_valve.valve_opening[0].value)
        print('valve opening fwh6=', fs.fwh6_valve.valve_opening[0].value)
        print('valve opening cond=', fs.cond_valve.valve_opening[0].value)
        print('valve opening hotwell rej=', fs.hotwell_rejection_valve.valve_opening[0].value)
        print('valve opening da rej=', fs.da_rejection_valve.valve_opening[0].value)
        print('valve opening makeup=', fs.makeup_valve.valve_opening[0].value)
        print('valve opening spray=', fs.spray_valve.valve_opening[0].value)
        print('valve opening bfp turb=', fs.bfp_turb_valve.valve_opening[0].value)
        print('valve opening throttle=', fs.turb.throttle_valve[1].valve_opening[0].value)
        print('fwh2 level=', fs.fwh2.condense.level[0].value)
        print('fwh3 level=', fs.fwh3.condense.level[0].value)
        print('fwh5 level=', fs.fwh5.condense.level[0].value)
        print('fwh6 level=', fs.fwh6.condense.level[0].value)
        print('hotwell level=', fs.hotwell_tank.tank_level[0].value)
        print('da level=', fs.da_tank.tank_level[0].value)
        print('cond_pump deltaP=', fs.cond_pump.deltaP[0].value)
        print('cond_pump work=', pyo.value(fs.cond_pump.work_mechanical[0]))
        print('cond_valve deltaP=', fs.cond_valve.deltaP[0].value)
        print('hotwell rejection flow=', fs.hotwell_rejection_valve.outlet.flow_mol[0].value)
        print('da rejection flow=', fs.da_rejection_valve.outlet.flow_mol[0].value)
        print('makeup flow=', fs.makeup_valve.outlet.flow_mol[0].value)
        print('spray flow=', fs.spray_valve.outlet.flow_mol[0].value)
        print('cond split fraction 1=', fs.cond_split.split_fraction[0,"outlet_1"].value)
        print('cond split fraction 2=', fs.cond_split.split_fraction[0,"outlet_2"].value)
        print('cond split fraction 3=', fs.cond_split.split_fraction[0,"outlet_3"].value)
        print('spray split fraction=', fs.split_attemp.split_fraction[0,"Spray"].value)
        print('outlet stage inlet temp=', pyo.value(fs.turb.outlet_stage.control_volume.properties_in[0].temperature))
        print('outlet stage inlet pres=', pyo.value(fs.turb.outlet_stage.control_volume.properties_in[0].pressure))
        print('outlet stage inlet flow_mol=', pyo.value(fs.turb.outlet_stage.control_volume.properties_in[0].flow_mol))
        print('outlet stage outlet flow_vol=', pyo.value(fs.turb.outlet_stage.control_volume.properties_out[0].flow_vol))
        print('outlet stage inlet vapor frac=', pyo.value(fs.turb.outlet_stage.control_volume.properties_in[0].vapor_frac))
        print('outlet state outlet vapor frac=', pyo.value(fs.turb.outlet_stage.control_volume.properties_out[0].vapor_frac))
        print('outlet stage efficiency=', pyo.value(fs.turb.outlet_stage.efficiency_isentropic[0]))
        print('inlet stage efficiency=', pyo.value(fs.turb.inlet_stage[1].efficiency_isentropic[0]))
        print('HP stage 1 efficiency=', pyo.value(fs.turb.hp_stages[1].efficiency_isentropic[0]))
        print('IP stage 1 efficiency=', pyo.value(fs.turb.ip_stages[1].efficiency_isentropic[0]))
        print('LP stage 1 efficiency=', pyo.value(fs.turb.lp_stages[1].efficiency_isentropic[0]))
        print('Tel outlet stage=', pyo.value(fs.turb.outlet_stage.tel[0]))
        print('delta_h =', pyo.value(fs.turb.outlet_stage.delta_enth_isentropic[0]))
        print('outlet stage power =', pyo.value(fs.turb.outlet_stage.power_shaft[0]))
        print('inlet stage pressure ratio=', pyo.value(fs.turb.inlet_stage[1].ratioP[0]))
        print('bfp turb valve inlet pressure=', fs.bfp_turb_valve.inlet.pressure[0].value)
        print('bfp turb inlet pressure=', fs.bfp_turb.inlet.pressure[0].value)
        print('bfp turb outlet flow=', fs.bfp_turb.outlet.flow_mol[0].value)
        print('bfp turb outlet enth=', fs.bfp_turb.outlet.enth_mol[0].value)
        print('bfp turb outlet pres=', fs.bfp_turb.outlet.pressure[0].value)
        print('bfp turb work=', fs.bfp_turb.control_volume.work[0].value)
        print('bfp turb efficiency=', fs.bfp_turb.efficiency_isentropic[0].value)
        print('bfp turb os outlet flow=', fs.bfp_turb_os.outlet.flow_mol[0].value)
        print('bfp turb os outlet enth=', fs.bfp_turb_os.outlet.enth_mol[0].value)
        print('bfp turb os outlet pres=', fs.bfp_turb_os.outlet.pressure[0].value)
        print('bfp turb os outlet vapor frac=', pyo.value(fs.bfp_turb_os.control_volume.properties_out[0].vapor_frac))
        print('bfp turb os work=', fs.bfp_turb_os.control_volume.work[0].value)
        print('bfp turb os efficiency=', fs.bfp_turb_os.efficiency_isentropic[0].value)
        print('Tel turb os =', pyo.value(fs.bfp_turb_os.tel[0]))
        print('bfp turb os flow coef=', fs.bfp_turb_os.flow_coeff.value)
        print('bfp turb os out vol flow=', pyo.value(fs.bfp_turb_os.control_volume.properties_out[0].flow_vol))

        _log.info("Adding heat transfer coefficent correations...")
        _add_heat_transfer_correlation(fs)
        print('cooling water flow=', pyo.value(fs.condenser.inlet_2.flow_mol[0]))
        print('U0 after solving= ', fs.fwh6.condense.U0[0].value)
    else:
        _add_heat_transfer_correlation(fs)
